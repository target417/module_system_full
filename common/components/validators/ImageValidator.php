...
class ImageValidator extends CFileValidator{
	...
	/**
	* @var boolean Указывает на то, обязатен ли файл для загрузки.
	*/
	...
	protected function validateFile($object, $attribute, $file)
	{
		...
		if($this->minWidth !== null && $size[0] < $this->minWidth){
			$message=$this->tooSmallWidth ? $this->tooSmallWidth : Yii::t('yii','Изображение должно быть не меньше {width}px в ширину');
			$this->addError($object,$attribute,$message,array('{width}'=>$this->minWidth));
		}
		
		if($this->maxWidth !== null && $size[0] > $this->maxWidth){
			$message=$this->tooLargWidth ? $this->tooLargWidth : Yii::t('yii','Изображение должно быть не больше {width}px в ширину');
			$this->addError($object,$attribute,$message,array('{width}'=>$this->maxWidth));
		}
		
		if($this->minHeight !== null && $size[1] < $this->minHeight){
			$message=$this->tooSmallHeight ? $this->tooSmallHeight : Yii::t('yii','Изображение должно быть не меньше {height}px в высоту');
			$this->addError($object,$attribute,$message,array('{height}'=>$this->minHeight));
		}
		
		if($this->maxHeight !== null && $size[1] > $this->maxHeight){
			$message=$this->tooLargHeight ? $this->tooLargHeight : Yii::t('yii','Изображение должно быть не больше {height}px в ширину');
